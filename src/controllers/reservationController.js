...
exports.createReservation = async (req, res) => {
    const { guest_fullname, guest_phone, guest_email, guest_address, guest_type_id, check_in, check_out, room_type_id, number_of_rooms, adults, children, reservation_note } = req.body;

    if (!guest_fullname || !guest_phone || !guest_email || !check_in || !check_out || !number_of_rooms || !room_type_id) {
  ...
  try {
    ...
    const query = 'INSERT INTO reservations (guest_fullname, guest_phone, guest_email, guest_address, guest_type_id, check_in, check_out, number_of_rooms, adults, children, reservation_note, recommended_rooms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    const values = [guest_fullname, guest_phone, guest_email, guest_address, guest_type_id, check_in, check_out, number_of_rooms, adults, children, reservation_note, suitableRooms.join(',')];
...
exports.updateReservation = async (req, res) => {
    ...
    const { reservation_note, declined_reason, assigned_room, status } = req.body;
    ...
    try {
        const query = 'UPDATE reservations SET reservation_note=?, declined_reason=?, assigned_room=?, status=? WHERE reservation_id = ?';
        const values = [reservation_note, declined_reason, assigned_room, status, reservation_id];
        ...
        if(status === 'Confirmed') {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
                const valuesBooking = [
                    reservation.guest_fullname, reservation.guest_id_card, reservation.guest_phone, 
                    reservation.guest_email, reservation.guest_address, reservation.guest_type_id, 
                    reservation.check_in, reservation.check_out, roomID, reservation.room_type_id, 
                    reservation.adults, reservation.children, 'Due In'
                ];
    
                const [bookingResult] = await db.promise().query(queryBooking, valuesBooking);
            ...
            [reservation.guest_fullname, reservation.guest_id_card, reservation.guest_type_id, bookingId, roomID || null]
            );
            }
    ...
    transporter.sendMail(mailOptions, (error) => {
        ...
        res.json({ message: 'Reservation updated and email sent successfully' });
    });
}
